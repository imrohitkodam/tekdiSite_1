...
if (!empty($feed) && is_string($feed)) {
    ...
    if ($feed !== false) {
        ?>
        <div style="direction: <?php echo $rssrtl ? 'rtl' : 'ltr'; ?>;" class="text-<?php echo $rssrtl ? 'right' : 'left'; ?> feed mena-buzz-view ">
        <?php
        // Feed title
        ...
        <!-- Show items -->
        <?php if (!empty($feed)) { ?>
            <div class="featured-news-list mod-list course-cover row ">
                <?php for ($i = 0, $max = min(count($feed), $params->get('rssitems', 3)); $i < $max; $i++) { ?>
                <div class="col-sm-6 mb-4">
                   <div class="course-card-pin tjlmspin card-zoomin ">
                        <?php
                        ...
                        <div class="row mx-0">
                            <div class="col-sm-5 px-0">
                                <div class="p-0 br-0 tjlmspin__thumbnail h-100">
                                    <a href="<?php echo $feed[$i]->uri; ?>" target="_blank" class="center d-block feed-bg-img" style="background-image: url('<?php echo $feed[$i]->links[0]->uri; ?>');">
                                    </a>
                                </div>
                            </div>
                            <div class="col-sm-7">
