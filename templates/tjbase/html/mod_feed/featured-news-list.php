...
if (!empty($feed) && is_string($feed)) {
    ...
    if ($feed !== false) {
        ?>
        <div style="direction: <?php echo $rssrtl ? 'rtl' : 'ltr'; ?>;" class="text-<?php echo $rssrtl ? 'right' : 'left'; ?> feed course-module-home">
        <?php
        // Feed title
        ...
        <!-- Show items -->
        <?php if (!empty($feed)) { ?>
            <div class="featured-news-list mod-list course-cover">
                <?php for ($i = 0, $max = min(count($feed), $params->get('rssitems', 3)); $i < $max; $i++) { ?>
                <div class=" course-card-pin float-start tjlmspin card-zoomin">
                    ...
                    ?>
                     <div class="p-0 br-0 tjlmspin__thumbnail h-100">
                            <a href="<?php echo $feed[$i]->uri; ?>" target="_blank" class="center h-100 d-block" style="background-image: url('<?php echo $feed[$i]->links[0]->uri; ?>');">
                            </a>
                    </div>
                    <div class="card-section-bottom">
                        <div class="caption p-3">
                            <div class="tjlmspin__caption">
                                <!-- <div class="mb-2">
                                    <img  src="<?php echo $feed[$i]->links[0]->uri; ?> " class="course-thumbnail-image"/>
                                </div> -->
								<div class="d-lg-inline-block mb-1 start-date">
                                    <?php echo date('d M Y',strtotime($feed[$i]->publishedDate));?>
                                </div>
                              	<div class="d-lg-inline-block mb-1 start-date float-end category">
                                    <?php echo array_keys($feed[$i]->categories)[0]; ?>
                                </div>
                                <h4 class="tjlmspin__caption_title mt-3">
                                    <?php if ($feed[$i]->title !== '') : ?>
                                        <a href="<?php echo $feed[$i]->uri; ?>" target="_blank">
