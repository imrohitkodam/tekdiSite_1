...
/**
 * Helper for Similar Tags with introtext and images
 *
 * @package     Joomla.Site
 * @subpackage  mod_tags_similar_wci
 * @since       3.1
 */
abstract class ModTagssimilarWciHelper
{
		...
		if ($option != 'com_tags' && $view != 'category'  && $option != 'com_users') {
			...
			$query = $db->getQuery(true)
				...
				array(
					...
					$db->quoteName('cc.core_created_time')
					)
			);
			...
			switch($sort){
				case "up":
					$query->order($db->quoteName('cc.core_created_time') .'ASC');
					break;
				case "down":
					$query->order($db->quoteName('cc.core_created_time') .'ASC');
					break;				
